...
const Dashboard: React.FC = () => {
  ...
  const lowStockItems = insumos.filter(i => i.stockActual <= i.stockMinimo).length;
  const totalValue = insumos.reduce((sum, i) => sum + (i.stockActual * i.precioUnitario), 0);
  const outOfStockItems = insumos.filter(i => i.stockActual === 0).length;
  ...
  const recentLowStock = insumos.filter(i => i.stockActual <= i.stockMinimo).slice(0, 5);
